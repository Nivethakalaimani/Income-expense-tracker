...
/*
const dummy=[
    { id:1, description: "salary", amount:32525},
...
function updateamt(){
    ...
    const inm=amounts.filter((item)=>item>0).reduce((acc,item)=>(acc+=item),0).toFixed(2);
    incamt.innerHTML=`Rs : ${inm}`;
    const exps=amounts.filter((item)=>item<0).reduce((acc,item)=>(acc+=item),0).toFixed(2);
